...
TileSet* j1Map::GetTilesetFromTileId(int id) const
{
	...
	while (tileset_iterator) {
		if (id < tileset_iterator->data->firstgid) {
			return tileset_iterator->data;
		}
		tileset_iterator = tileset_iterator->next;
	}
	return tileset_iterator->data;

	//for (p2List_item<TileSet*>* tileset_iterator = data.tilesets.start; tileset_iterator; tileset_iterator = tileset_iterator->next) {
	//	if (!tileset_iterator->next) {
	//		return (tileset_iterator->data);
	//	}
	//	if (id >= tileset_iterator->data->firstgid && id < tileset_iterator->next->data->firstgid) {
	//		return (tileset_iterator->data);
	//	}
	//}
...
bool j1Map::LoadProperties(pugi::xml_node& node, Properties& properties)
{
	...
	// an xml_node
	properties.draw = node.child("properties").child("property").attribute("value").as_bool();
